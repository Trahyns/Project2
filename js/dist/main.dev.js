...
$(function (params) {
  ...
  $('#inlineFormInputGroup').focus(); //Create the API URL from the user inputs, API Key and Pagination
  ...
  $("#submit").click(function () {
    query = $('#inlineFormInputGroup').val(); //Display error if no input entered

    if (query.length == 0 && flag == 0) {
      $(".form-inline").effect("shake");
      $("#inlineFormInputGroup").addClass("error");
    } else {
      $("#inlineFormInputGroup").removeClass("error"); //Generate entire URL from the user input
...
function gotData(data) {
  ...
  if (data.meta.status) {
    if (data.data.length > 0) {
      $(".gif-results").show();
      $('.no-result').hide(); //Remove any existing data - used to clearing out previous searches
      ...
      $('.loading-gif').css("display", "block"); //Iterate through all of the returned JSON data and append to HTML 

      for (var i = 0; i < data.data.length; i++) {
        //Get the URL from JSON
        var gifURL = data.data[i].images.original.url; // var giphyURL = data.data[i].url;
...
$("form").submit(function (e) {
  $('#submit').click(); //Blur out of the search bar after enter key hit
